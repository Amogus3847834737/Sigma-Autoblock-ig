...
local function triggerAction()
    if tick() - lastPress > SETTINGS.DebounceTime then
        ...
        task.spawn(function()
            if keypress then
                keyrelease(0x51); task.wait(0.05); if keyrelease then keyrelease(0x51) end
...
local STATE = {
    ...
    InvertAxis = false -- The Toggle State
}
...
ScreenGui.Name = "CombatAssist_UltimateFix"
...
MainFrame.Size = UDim2.new(0, 200, 0, 240) -- Taller for Fix Button
...
local function createBtn(text, order)
    ...
    btn.Position = UDim2.new(0.05, 0, 0.15 + (order * 0.18), 0)
...
local FixBtn = createBtn("Fix Axis: Default", 3.2)
FixBtn.BackgroundColor3 = Color3.fromRGB(50, 50, 200) -- Blue
...
FixBtn.MouseButton1Click:Connect(function()
    STATE.InvertAxis = not STATE.InvertAxis
    FixBtn.Text = STATE.InvertAxis and "Fix Axis: INVERTED" or "Fix Axis: Default"
end)
...
JoyBack.Position = UDim2.new(0, 50, 1, -100) 
JoyBack.Image = "rbxassetid://8004055268"
JoyBack.ImageTransparency = 1 
...
RunService.RenderStepped:Connect(function()
    ...
    if hum and root then
        ...
        if STATE.JoystickEnabled and isDragging and moveInput.Magnitude > 0 then
            ...
            -- [[ THE FIX ]] --
            local yInput = moveInput.Y
            if STATE.InvertAxis then
                yInput = -yInput -- Flip it if button is clicked
            end
            
            -- Logic: Negative Y is usually UP on screen
            local forwardStrength = -yInput 
            local rightStrength = moveInput.X
